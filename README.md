...
기존 양팔과 독립적인 레일 장착 PiPER를 실행하려면 다음 명령을 사용한다.

```bash
./run/rail_mani.sh
```

레일 전용 장면이 열리므로 기존 고정 팔 두 대와 그 액추에이터는 `Controls`에도
나타나지 않는다. cuRobo는 `rail_slide` 1축과 팔 관절 6축만 묶은 독립 7-DOF
문제를 푼다. 주황색 목표를 옮기면 빨간 TCP 경로와 반투명한 IK 도착 자세가
표시되고 `Space`를 눌러 실행한다. `Ctrl+Z`/`Ctrl+Shift+Z`로 목표 이동을
되돌리거나 다시 적용할 수 있다. PiPER는 천장 레일 아래에 180도 반전되어
장착된다. 레일 축은 동역학으로 미끄러지는 축이 아니라 계획 위치를 매 프레임
직접 고정하는 이상적인 kinematic 축이므로 관성 이동이나 정지 오버슈트가 없다.
