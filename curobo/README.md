...
├── robots/piper_rail/       # 독립 1축 레일 + PiPER 7-DOF URDF
...
기존 양팔과 분리된 천장 레일 PiPER는 다음 명령으로 실행합니다.

```bash
cd /home/airlab/.keys/workspace/sim
./run/rail_mani.sh
```

주황색 목표를 드래그하면 `rail_slide + joint1~joint6`을 하나의 7-DOF 상태로
계획합니다. 빨간 미리보기 경로와 반투명 IK 도착 자세를 확인하고 `Space`를
누르면 실행됩니다. `Ctrl+Z`는 목표 이동을 되돌리고 `Ctrl+Shift+Z`는 다시
적용합니다. 레일 전용 MuJoCo scene을 사용하므로 `Controls`에는 레일과 해당
PiPER 액추에이터만 표시됩니다. 레일은 x축 `-0.40~+0.40 m` 범위이며 기존 두
팔의 12-DOF/6+6-DOF 플래너와 충돌 월드, 목표, 관절 상태를 공유하지 않습니다.
PiPER 베이스는 레일 아래에서 180도 상하 반전되어 있고, cuRobo URDF와 천장
충돌 월드에도 같은 변환이 적용됩니다. MuJoCo GUI에서 레일 위치는 직접 고정되어
관성, 밀림, 오버슈트 없이 계획 좌표에서 즉시 멈춥니다.
...
기본 화면에는 바닥(테이블), 서로 마주보는 두 팔, 초록 공통 목표점과 팔별
파생 목표점인 청록색·자홍색 마커가 표시됩니다. ㅁ자 게이트, C-space 점군과
파란 도달 구는 숨겨진 상태로 시작합니다. 통합 목표만 보고 싶으면
`--hide-derived-targets` 옵션을 사용합니다.
...
목표이며 이것만 드래그합니다. 기본 팔과 두 번째 팔의 목표는 공통점 기준 x축
양쪽 3 cm에 배치되어 두 목표 사이의 전체 간격은 6 cm입니다.
`--dual-goal-offset`으로 중심에서 각 팔 목표까지의 거리를 변경할 수 있습니다.
한 번의 공통 목표 변경으로 두
...
두 팔 사이의 목표 간격은 기본 6 cm입니다. 팔 하나당 중심점으로부터 떨어진
x축 거리를 변경하려면 다음처럼 지정합니다. 아래의 `0.06`은 팔당 거리이므로
두 목표 사이의 전체 간격은 12 cm가 됩니다.
...
cuRobo의 그리퍼 충돌 형상은 TCP 중심의 큰 단일 구가 아니라 MuJoCo 손가락
형상에 맞춘 작은 구들로 구성됩니다. 각 그리퍼 몸체는 8개 구, 끝 접촉 패드는
2개 구로 분리됩니다. 두 팔의 접촉 패드끼리만 self-collision 예외이며 손목,
그리퍼 몸체와 나머지 팔 사이 충돌은 계속 검사합니다. 생성된 궤적은 실행 전에
MuJoCo 실제 충돌 형상으로 한 번 더 검사됩니다.

게이트와 C-space 표시를 처음부터 켜려면 다음 옵션을 조합해 사용합니다.
